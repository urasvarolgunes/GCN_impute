...
def distanceAnchorEuclidean(x, anchor_index, n_jobs=-1, func=dis_base):
    '''
...
def anchorKNN(dis, k, anchor_index, n_jobs=-1, func=knn_base):
    if (k > dis.shape[1]-1):
    ...
    sub_list = np.array_split(range(dis.shape[0]), n_jobs)
    processes = []
    return_dic = mp.Manager().dict()
...
def nnlsw(x, graph, pid, sub_list, return_dic, epsilon=None):
    '''
    nonnegative least saqure
    represent a point as a linear combination of its knn
    '''
    n = graph.shape[1]
    r = len(sub_list)
    ...
    for i in range(r):
        ...
        gvec = graph[ind_i]
        indK = [j for j in range(n) if gvec[0,j] == 1]
        k = len(indK)
...
def multicore_nnls(x, graph, n_jobs=-1, epsilon=None, func=nnlsw):
    total_cpu = mp.cpu_count()
    if type(n_jobs) is not int or n_jobs < -1 or n_jobs > total_cpu:
        print("Specify correct job number!")
        exit(0)
    ...
    graph_list = np.array_split(range(graph.shape[0]), n_jobs)
    processes = []
    return_dic = mp.Manager().dict()
...
if __name__ == "__main__":
    ...
    dis = distanceAnchorEuclidean(X, anchor_index, n_jobs=-1)
    print("distance mat: %.2f s" % (time.time()-start))
    G = anchorKNN(dis, k, anchor_index, n_jobs=-1)
    print("KNN construction: %.2f s" % (time.time()-start))
    
    #W = multicore_nnls(X, G, n_jobs=1) #don't use this for large graphs
